...
public class KohonenNetwork extends Network implements Serializable {
   ...
   /**
	 * 
	 */
	private static final long serialVersionUID = -8139696077833132556L;
...
/**
   ...
   public long duration; // in second;
...
// just in case it gets too small
    ...
    */
   public native int winner_native(double input[], double normfac[], double synth[]);
   ...
   public int winner(double input[] ,double normfac[] ,double synth[])
   {
     ...
     for (int i=0 ; i<outputNeuronCount; ++i ) {
       optr = outputWeights[i];
       output[i] = (dotProduct (input , optr) * normfac[0] + synth[0] * optr[inputNeuronCount] + 1) * 0.5;
       // Remap to bipolar(-1,1 to 0,1)
//       output[i] = 0.5 * (output[i] + 1.0) ;
       if ( output[i] > biggest ) {
         biggest = output[i] ;
...
// main loop:
     ...
     for ( iter=0 ; ; iter++ ) {
       ...
       System.out.println("error: " + bigerr[0]);
       PrintWriter writer = null;
		
		try {
			writer = new PrintWriter("./training_progress.txt", "UTF-8");		
			writer.println("error: " + bigerr[0]);
